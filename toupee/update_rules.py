...
class SGD(UpdateRule):
    ...
    def set_defaults(self):
        self._default_value('momentum',0.)
        self._default_value('momentum_decay',1.)
        self._default_value('renorm',False)
...
class Adam(UpdateRule):
    ...
    def set_defaults(self):
        self._default_value('beta1',0.9)
        self._default_value('beta2',0.999)
...
class ARMSProp(UpdateRule):

    yaml_tag = u'!ARMSProp'

    def __new__(cls):
        instance = super(ARMSProp,cls).__new__(cls)
        common.toupee_global_instance.add_epoch_hook(lambda x: instance.epoch_hook(x))
        common.toupee_global_instance.add_reset_hook(lambda x: instance.reset(x))
        return instance
    ...
    def set_defaults(self):
        self._default_value('momentum',0.)
        self._default_value('momentum_decay',1.)
        self._default_value('eta_plus',1.1)
        self._default_value('eta_minus',0.9)
...
#        acceleration = theano.printing.Print("ACC")(acceleration)
        ...
        updates.append((self.acceleration_mean,acceleration_mean))
    ...
    def serialize(self):
        self.set_defaults()
        return 'ARMSProp {{ momentum: {0} }}'.format(self.momentum)


class ARMSProp2(UpdateRule):

    yaml_tag = u'!ARMSProp2'

    def __new__(cls):
        instance = super(ARMSProp2,cls).__new__(cls)
        common.toupee_global_instance.add_epoch_hook(lambda x: instance.epoch_hook(x))
        common.toupee_global_instance.add_reset_hook(lambda x: instance.reset(x))
        return instance
...
#        acceleration = theano.printing.Print("ACC")(acceleration)
        ...
        m_ = m / (1 - (self.beta1 ** self.t))
        v_ = v / (1 - (self.beta2 ** self.t))
        scaled_delta_w = m_ / (T.sqrt(v_) + epsilon)
        learning_rate = - learning_rate.get() * acceleration / acceleration_mean
        delta_w = scaled_delta_w * mask * learning_rate
        new_w = param + delta_w
        updates.append((self.acceleration,acceleration))
        updates.append((self.acceleration_mean,acceleration_mean))
        updates.append((previous_grad,gparam))
